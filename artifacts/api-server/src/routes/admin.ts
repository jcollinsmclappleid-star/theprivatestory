...
// ---------------------------------------------------------------------------
// POST /admin/chat — streaming AI chat with full platform context
// ---------------------------------------------------------------------------

function buildChatSystemPrompt(): string {
  const arcFramework = [
    `THE FIVE EPISODE EROTIC ARC — ${FIVE_EPISODE_EROTIC_ARC.framework.name}`,
    `Philosophy: ${FIVE_EPISODE_EROTIC_ARC.framework.philosophy}`,
    `Golden Rule: ${FIVE_EPISODE_EROTIC_ARC.framework.golden_rule}`,
    "",
    "EPISODE BLUEPRINT:",
    ...FIVE_EPISODE_EROTIC_ARC.episodes.map((ep) =>
      [
        `Episode ${ep.number}: ${ep.name}  |  Explicit: ${ep.explicit_level}  |  Length: ${ep.word_count}`,
        `Function: ${ep.function}`,
        `Emotional Job: ${ep.emotional_job}`,
        `Mandatory: ${ep.mandatory.join(" | ")}`,
      ].join("\n")
    ),
  ].join("\n\n");

  const seriesSummaries = SERIES_CATALOG.map((s) =>
    [
      `SERIES: ${s.title}  (id: ${s.id})`,
      s.description,
      `Female Lead: ${s.femaleLead}`,
      `Male Lead: ${s.maleLead}`,
      `Arc: ${s.seriesArc}`,
      `Episodes: ${s.episodes.map((ep, i) => `${i + 1}. "${ep.title}" — ${ep.dynamic} [${ep.intensity}]`).join(" | ")}`,
    ].join("\n")
  ).join("\n\n");

  return [
    "You are the editorial AI for The Private Story — a premium adult audio erotica platform.",
    "You have complete knowledge of the platform's writing rules, five-series catalog, and the Five Episode Erotic Arc framework.",
    "Answer questions about story quality, series arcs, episode structure, writing guidance, characters, and platform content.",
    "Be direct, specific, and knowledgeable. You are talking to the admin/editor of the platform.",
    "When asked to draft or improve content, apply the full MASTER EROTIC LAYER and arc rules.",
    "",
    "══════════════════════════════════════════",
    "MASTER WRITING RULES:",
    "══════════════════════════════════════════",
    MASTER_EROTIC_LAYER.trim(),
    "",
    "══════════════════════════════════════════",
    "FIVE EPISODE ARC FRAMEWORK:",
    "══════════════════════════════════════════",
    arcFramework,
    "",
    "══════════════════════════════════════════",
    "SERIES CATALOG (5 series):",
    "══════════════════════════════════════════",
    seriesSummaries,
  ].join("\n");
}
